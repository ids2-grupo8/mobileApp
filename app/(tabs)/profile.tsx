...
function ProfileSkeleton({ topPad, C }: { topPad: number; C: ThemeColors }) {
  const sk = {
    root: { flex: 1, backgroundColor: C.bg, paddingHorizontal: 20, paddingTop: topPad + 24 },
    userCard: { flexDirection: 'row' as const, gap: 16, alignItems: 'center' as const, marginBottom: 24 },
  };
  return (
    <View style={sk.root}>
      <View style={sk.userCard}>
        <SkeletonBox width={72} height={72} borderRadius={36} skeletonColor={C.skeletonBase} />
      ...
      <SkeletonBox width="100%" height={72} borderRadius={16} style={{ marginBottom: 12 }} skeletonColor={C.skeletonBase} />
      {[0, 1, 2, 3].map((i) => (
        <SkeletonBox key={i} width="100%" height={52} borderRadius={14} style={{ marginBottom: 8 }} skeletonColor={C.skeletonBase} />
      ))}
    </View>
  );
}
...
function Avatar({ name, size = 72, C }: { name: string; size?: number; C: ThemeColors }) {
  ...
  return (
    <View style={{
      ...
      <Text style={{ color: C.accent, fontWeight: '700', fontSize: size * 0.34 }}>{initials}</Text>
    </View>
  );
}
  ...
  return (
    <TouchableOpacity
      ...
      <View style={s.mrRight}>
        {rightLabel ? (
          <Text style={[s.mrRightLabel, { color: C.textMuted }]}>{rightLabel}</Text>
        ) : null}
  ...
  return (
    <View style={[s.root, { paddingTop: insets.top, backgroundColor: C.bg }]}>
      <ScrollView
        ...
        {/* ── User card ── */}
        <View style={[s.userCard, { backgroundColor: C.card, borderColor: C.border }]}>
